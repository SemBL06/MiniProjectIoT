...
const char* WIFI_SSID = "SappigeWifi"; // Fill wifi SSID between quotes
const char* WIFI_PASS = "SappigeTijden"; // Fill wifi pass between quotes

//MQTT Information 
const char* MQTT_SERVER = "mqtt.iote-ap.be"; // DO NOT CHANGE!
const char* MQTT_ID = "9486446874684"; // Use random id to connect ot mqtt server
...
void driveMotors(int leftMotorDirection, int leftMotorSpeed, int rightMotorDirection, int rightMotorSpeed) {
  // Control Left Motor (Channels 0 and 1)
  if (leftMotorDirection == MOTOR_FORWARD) {
    ledcWrite(MOTOR_CHANNELS[0], leftMotorSpeed);  // Forward channel active
    ledcWrite(MOTOR_CHANNELS[1], 0);                // Backward channel off
  } 
  else if (leftMotorDirection == MOTOR_BACKWARDS) {
    ledcWrite(MOTOR_CHANNELS[0], 0);                // Forward channel off
    ledcWrite(MOTOR_CHANNELS[1], leftMotorSpeed);  // Backward channel active
  } 
  else { // MOTOR_STOP
    ledcWrite(MOTOR_CHANNELS[0], 0);                // Both channels off
    ledcWrite(MOTOR_CHANNELS[1], 0);
  }

  // Control Right Motor (Channels 2 and 3)
  if (rightMotorDirection == MOTOR_FORWARD) {
    ledcWrite(MOTOR_CHANNELS[2], rightMotorSpeed); // Forward channel active
    ledcWrite(MOTOR_CHANNELS[3], 0);                // Backward channel off
  } 
  else if (rightMotorDirection == MOTOR_BACKWARDS) {
    ledcWrite(MOTOR_CHANNELS[2], 0);                // Forward channel off
    ledcWrite(MOTOR_CHANNELS[3], rightMotorSpeed); // Backward channel active
  } 
  else { // MOTOR_STOP
    ledcWrite(MOTOR_CHANNELS[2], 0);                // Both channels off
